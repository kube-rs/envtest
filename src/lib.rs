...
#[cfg(docsrs)]
trait EnvTest {
    /// Creates the environment and returns the [`Server`] instance. Cluster exists for the lifetime of the [`Server`].
    fn create(req: Environment) -> CreateResponse;
    /// Destroys the environment based on the provided kubeconfig file.
    fn destroy(kubeconfig: String) -> DestroyResponse;
}

#[derive(Default)]
#[cfg_attr(not(docsrs), derive(rust2go::R2G))]
